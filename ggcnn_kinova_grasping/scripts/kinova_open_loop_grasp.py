...
def execute_grasp():
    ...
    print(type(msg.data))
    # Fei Added
    #msg = [0.7716618776321411, 0.08534491807222366, -0.004996682051569223, 1.254616379737854, 58.95121765136719, 205.00335693359375, 0.0, 206.45065307617188, 85.0, 105.0]
    d = list(msg.data)
    ...
    gp_base = convert_pose(gp, 'camera_depth_optical_frame', 'j2s7s300_link_base')
    ...
    publish_pose_as_transform(gp_base, 'j2s7s300_link_base', 'G', 0.5)
    ...
    print('gp_base')
    print(gp_base)
    raw_input('Press Enter to Move to target position')
    # rospy.sleep(0.5)
    ...
    raw_input('Press Enter to Move back up to initial position')
    rospy.sleep(0.5)
    move_to_pose(gp_base)
